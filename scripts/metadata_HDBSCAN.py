...
import numpy as np
import pandas as pd
from sklearn.metrics import normalized_mutual_info_score, adjusted_rand_score, homogeneity_completeness_v_measure, adjusted_mutual_info_score
...
if __name__ == "__main__":
    ...
    true_vals = clusters[args.true_clusters_column].values
    predicted_vals = clusters[args.predicted_clusters_column].values
    output_df = pd.DataFrame([{
        "method": args.method,
        "predicted_clusters_column": args.predicted_clusters_column,
        "normalized_vi": np.round(normalized_vi, 4),
        ...
        "normalized_mutual_info_score": normalized_mutual_info_score(true_vals,predicted_vals),
        "adjusted_rand_score": adjusted_rand_score(true_vals,predicted_vals),
        "homogeneity_completeness_v_measure": homogeneity_completeness_v_measure(true_vals,predicted_vals)[-1],
        "adjusted_mutual_info_score": adjusted_mutual_info_score(true_vals,predicted_vals)
